...
async def list_schedules(
    ...
    if start_date is not None:
        ...
        stmt = stmt.where(Schedule.scheduled_at >= start_dt)

    if end_date is not None:
        end_dt = datetime.combine(end_date, time.max, tzinfo=UTC)
        stmt = stmt.where(Schedule.scheduled_at <= end_dt)
...
def _build_schedule_response(schedule: Schedule, company_name: str | None) -> ScheduleResponse:
    ...
    return response.model_copy(update={"company_name": company_name})
